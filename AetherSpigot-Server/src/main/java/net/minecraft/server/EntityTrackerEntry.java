...
public class EntityTrackerEntry {
    ...
    public int b;
    public int updateFrequency; // AetherSpigot
    public int xLoc;
    public int yLoc;
    public int zLoc;
    public int yRot;
    ...
    public double k;
    public double l;
    public int updateCounter; // AetherSpigot
    private double q;
    private double r;
    private double s;
    ...
    public EntityTrackerEntry(Entity entity, int i, int j, boolean flag) {
        ...
        // AetherSpigot end
        this.yRot = Math.round(entity.yaw * 256.0F / 360.0F);
        this.xRot = Math.round(entity.pitch * 256.0F / 360.0F);
        this.lastHeadMotion = Math.round(entity.getHeadRotation() * 256.0F / 360.0F);
    ...
    public void track(List<EntityHuman> list) {
        ...
        if (this.w != this.tracker.vehicle || this.tracker.vehicle != null && this.updateCounter % 60 == 0) {
        ...
        if (this.tracker instanceof EntityItemFrame /*&& this.m % 10 == 0*/) { // CraftBukkit - Moved below, should always enter this block
            ...
            if (this.updateCounter % 10 == 0 && itemstack != null && itemstack.getItem() instanceof ItemWorldMap) { // CraftBukkit - Moved this.m % 10 logic here so item frames do not enter the other blocks
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.tracker.ai || this.tracker.getDataWatcher().a()) {
            int i;
            int j;
            ...
            if (this.tracker.vehicle == null) {
                ...
                int l = Math.round(this.tracker.yaw * 256.0F / 360.0F);
                int i1 = Math.round(this.tracker.pitch * 256.0F / 360.0F);
                // AetherSpigot end
                int j1 = i - this.xLoc;
                int k1 = j - this.yLoc;
                int l1 = k - this.zLoc;
                ...
                if (this.updateCounter > 0 || this.tracker instanceof EntityArrow) { // PaperSpigot - Moved up
                    ...
                    if (j1 >= -128 && j1 < 128 && k1 >= -128 && k1 < 128 && l1 >= -128 && l1 < 128 && this.ticksSinceLastForcedTeleport <= 400 && !this.ridingEntity && this.onGround == this.tracker.onGround) {
                        if ((!flag || !flag1) && !(this.tracker instanceof EntityArrow)) {
                            if (flag) {
                                packet = new PacketPlayOutEntity.PacketPlayOutRelEntityMove(this.tracker.getId(), (byte) j1, (byte) k1, (byte) l1, this.tracker.onGround);
                            } else if (flag1) {
                                packet = new PacketPlayOutEntity.PacketPlayOutEntityLook(this.tracker.getId(), (byte) l, (byte) i1, this.tracker.onGround);
                            }
                        } else {
                            packet = new PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook(this.tracker.getId(), (byte) j1, (byte) k1, (byte) l1, (byte) l, (byte) i1, this.tracker.onGround);
                        }
                    } else {
                        ...
                        // CraftBukkit end
                        packet = new PacketPlayOutEntityTeleport(this.tracker.getId(), i, j, k, (byte) l, (byte) i1, this.tracker.onGround);
                    }
                }
                    ...
                    if (packet instanceof PacketPlayOutEntityTeleport) {
                        ...
                        for (java.util.Map.Entry<EntityPlayer, Boolean> viewer : trackedPlayerMap.entrySet()) {
                            if (viewer.getValue()) {
                                ...
                                viewer.getKey().playerConnection.sendPacket((Packet) packet);
                            }
                        }
                    }
                    // PaperSpigot end
                ...
                i = Math.round(this.tracker.yaw * 256.0F / 360.0F);
                j = Math.round(this.tracker.pitch * 256.0F / 360.0F);
            ...
            i = Math.round(this.tracker.getHeadRotation() * 256.0F / 360.0F); // AetherSpigot
        ...
        if (this.tracker.velocityChanged) {
            ...
            if (this.tracker instanceof EntityPlayer) {
                ...
                // AetherSpigot start
                BowBoost boost = AetherSpigot.get().getBowBoost();
                boost.onVelocity(event);
                // AetherSpigot end
    ...
    private void sendMetadataToAllAssociatedPlayers() { // AetherSpigot
        ...
        if (entityplayer != this.tracker) {
            if (this.c(entityplayer)) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.e(entityplayer) || this.tracker.attachedToPlayer)) {
                    ...
                    entityplayer.playerConnection.sendPacket(new PacketPlayOutEntityHeadRotation(tracker, (byte) Math.round(tracker.getHeadRotation() * 256.0F / 360.0F))); // AetherSpigot
                    // CraftBukkit end
                    if (this.tracker instanceof EntityLiving) {
    ...
    private Packet c() {
        ...
        if (this.tracker instanceof EntityItem) {
            ...
            this.lastHeadMotion = MathHelper.d(this.tracker.getHeadRotation() * 256.0F / 360.0F);
