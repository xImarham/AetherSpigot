...
import xyz.aether.spigot.event.PlayerBlockHitEvent;
...
public abstract class EntityHuman extends EntityLiving {
    ...
    public void t_() {
        ...
        if (this.itemInUse != null) {
            ...
            if (itemstack == this.itemInUse) {
                ...
                if (--this.itemInUseCount == 0 && !this.world.isClientSide) {
                    this.s();
                }
            } else {
                this.clearItemInUse();
    ...
    protected void s() {
        if (this.itemInUse != null) {
            this.b(this.itemInUse, 16);
            int i = this.itemInUse.count;

            // AetherSpigot start
            if (world.aetherWorldConfig.eatWhileRunningBugFix && this instanceof EntityPlayer) {
                EntityPlayer entityPlayer = (EntityPlayer) this;
                if (this.itemInUse.m() == EnumAnimation.EAT) {
                    if (this.activeContainer.windowId != 0 || entityPlayer.isSprinting()) {
                        entityPlayer.playerConnection.sendPacket(new PacketPlayOutSetSlot(0,
                                this.activeContainer.getSlot(this.inventory, this.inventory.itemInHandIndex).index, this.itemInUse));
            ...
            if (event.isCancelled()) {
                ...
                if (this instanceof EntityPlayer) {
                    ((EntityPlayer) this).playerConnection.sendPacket(new PacketPlayOutSetSlot((byte) 0, activeContainer.getSlot((IInventory) this.inventory, this.inventory.itemInHandIndex).index, this.itemInUse));
                    // Spigot Start
    ...
    public EntityItem a(boolean flag) {
        if (this.isBlocking() && !world.aetherWorldConfig.dropSwordWhileBlocking) return null; // AetherSpigot
    ...
    protected boolean d(DamageSource damagesource, float f) { // void -> boolean
        ...
        if (!this.isInvulnerable(damagesource)) {
            if (!damagesource.ignoresArmor() && this.isBlocking() && f > 0.0F) {
                // AetherSpigot start - Fire PlayerBlockHitEvent
                PlayerBlockHitEvent blockEvent = new PlayerBlockHitEvent(
                        (Player) this.getBukkitEntity(),
                        damagesource.getEntity() != null ? damagesource.getEntity().getBukkitEntity() : null,
                        f,
                        (1.0F + f) * this.world.paperSpigotConfig.playerBlockingDamageMultiplier
                );
                this.world.getServer().getPluginManager().callEvent(blockEvent);
        ...
        // PandaSpigot end
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (this instanceof EntityPlayer && entity instanceof EntityPlayer) {
                       DamageListener.onPlayerAttack((EntityPlayer) this, (EntityPlayer) entity, flag2);
                    }
                    // AetherSpigot end
                    ...
                    if (flag2) {
                        if (entity instanceof EntityPlayer) {
                            if (((EntityPlayer) entity).knockbackProfile == null)
                                ((EntityPlayer) entity).knockbackProfile = AetherSpigot.get().getKnockbackHandler().getActiveProfile();;
