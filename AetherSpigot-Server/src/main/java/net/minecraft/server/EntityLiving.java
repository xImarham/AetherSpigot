    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (this.isInvulnerable(damagesource)) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                int damageTicks = this instanceof EntityPlayer ? ((EntityPlayer) this).knockbackProfile.getDamageTicks() : 20;

                if ((float) this.noDamageTicks > (float) damageTicks / 2.0F) {
                    return false;
                } else {
                    // CraftBukkit start
                    ...
                    this.lastDamage = f;
                    this.noDamageTicks = damageTicks;
                    // CraftBukkit end
                ...
                // PaperSpigot end
                    ...
                    if (entity != null) {
                        ...
                        this.aw = (float) (MathHelper.b(d0, d1) * 180.0D / Math.PI - (double) this.yaw);
        ...
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            if (this instanceof EntityPlayer) {
                if (((EntityPlayer) this).knockbackProfile == null)
                    ((EntityPlayer) this).knockbackProfile = AetherSpigot.get().getKnockbackHandler().getActiveProfile();
    ...
    protected boolean d(final DamageSource damagesource, float f) { // void -> boolean, add final
       if (!this.isInvulnerable(damagesource)) {
           ...
           Function<Double, Double> blocking = new Function<Double, Double>() {
               ...
               public Double apply(Double f) {
                   if (human) {
                       EntityPlayer victim = (EntityPlayer) EntityLiving.this;
                       if (!damagesource.ignoresArmor() && victim.isBlocking() && f > 0.0F) {
                           ...
                           PlayerBlockHitEvent blockEvent = new PlayerBlockHitEvent(
                                   ...
                                   f.floatValue(),
                                   (1.0F + f.floatValue()) * EntityLiving.this.world.paperSpigotConfig.playerBlockingDamageMultiplier
                           );
                           EntityLiving.this.world.getServer().getPluginManager().callEvent(blockEvent);
                           if (blockEvent.isCancelled()) {
                               return 0.0;
                           }
                           // AetherSpigot end
                           return -(f - ((1.0F + f.floatValue()) * EntityLiving.this.world.paperSpigotConfig.playerBlockingDamageMultiplier));
                       }
                   }
                   return -0.0;
               }
